...
The first run still has no modal tutorial, dimming, freeze, or altered physics: experienced players can attack the `男人勋章` immediately. An eligible desktop keyboard novice sees one quiet, dismissible lower-left action caption throughout the fresh countdown, so `SHIFT` is discoverable before control begins. It starts with `按住 SHIFT 漂移蓄能`, follows the real left rail to the yellow mark, changes to `松开 SHIFT` only when release can actually bank a diamond, retracts after the accepted charge, and reveals `SPACE 起飞` only when a stored cell and cyan branch make it useful. Eligibility is based on demonstrated banking knowledge rather than the unreliable run count: an unfinished novice can see it again after reload or retry, while an actual bank or an explicit close retires it persistently. Closing this small caption does not mark the bank action mastered and does not disarm the first-failure coach. Pressing Shift alone never advances it, it never appears on mobile, and deliberate gamepad input suppresses the keyboard copy.
