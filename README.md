...
The two strongest rivals form a real-input opening field rather than disappearing after flight two. Through the player's second scored flight, both receive bounded pace targets that keep them ahead and visibly chain genuine drift-release `BOOST` cycles; through the third, one protected rival remains ahead. The third pass atomically removes all player-gap pacing and technique pressure. Their fixed aggressive drift style may remain afterward, but competition is then authored and fair. No rival is teleported, assigned fake progress, made collision-immune, or allowed to slow the player.
