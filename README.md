...
Open the printed localhost URL. You start fourth in a six-racer challenge, with three rivals ahead and two behind. Opening contact is an occasional authored pack event rather than a guaranteed pile-up. Once the player proves a flight and opens a real gap, one primary rival, and only sometimes a second, may chase by performing the same drift-release `BOOST`, launch, and air-brake actions available to the player. They are never teleported into view and no catch is guaranteed.
...
The first run still has no modal tutorial, dimming, freeze, or altered physics: experienced players can attack the `男人勋章` immediately. An eligible desktop keyboard novice sees one quiet, dismissible lower-left navigation console throughout the fresh countdown, so `SHIFT` is discoverable before control begins. It starts with `按住 SHIFT 漂移蓄能`, follows the real left rail to the yellow mark, changes to `松开 SHIFT` only when release can actually bank a diamond, confirms the stored cell, then stays in the same location as a low-priority `SPACE` standby until the cyan launch vector makes takeoff actionable. The ordinary upper-right flight prompt uses the same keycap, clipped frame, type scale, and cyan accent on later runs. Eligibility is based on demonstrated banking knowledge rather than the unreliable run count: an unfinished novice can see it again after reload or retry, while an actual bank or an explicit close retires the full primer persistently. Closing this small console does not mark the bank action mastered and does not disarm the first-failure coach. Pressing Shift alone never advances it, it never appears on mobile, and deliberate gamepad input suppresses the keyboard copy.
