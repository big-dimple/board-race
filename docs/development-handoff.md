...
- `7be07fd` 已发布强敌链漂公平性与可读性改动：第四飞后用连续局部投影统一名次、AI、
  碰撞和画面中的赛道归属；对手技巧改为真实 Boat 状态驱动的释放脉冲，海面也改用连续
  法线与稀疏白浪。白色短放射线、灰烟和透明热雾锥体均已否决；当前本地版本为“真实漂移释放时，艇尾朝后上方的白芯蓝焰”，未提交、未发布。
