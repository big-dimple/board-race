...
- 天空保持原有三段渐变；近云继续使用近景层，远云改为 8 个低对比宽幅空气层，不再复用
  近景卡通云图标。可见太阳改为小圆盘、克制环光和不规则短射线，视觉方向略低于共享受光
  方向，因此只在部分追车视角出现，不改变艇体、水面或车手的受光合同。
...
- 选手的玩家可见名称已统一为 `GLM`、`ChatGPT`、`Gemini`、`Kimi`、`Claude`、`DeepSeek`；
...
第一阶段已完成：在原有单个海面 draw call 内加入连续的中尺度风浪法线和受光带，使用
`fwidth` 抗锯齿并在 `18-210m` 淡出；`waves.ts` 的 Gerstner 物理波形、船体采样、尾流和
航道所有权均未改变。后续若继续追求商业竞速级浪峰细节或高速掠过反馈，必须先证明不会
淹没漂移、对手姿态、艇尾技巧提示或路线判读；动作信息优先于水面炫技。
