...
- 水面荣誉目标是实体浮标式鸭子、海铃、星标、王冠和彗星，带浮筒、泡沫、桅杆和实心徽记；命中只
  记录 `center` / `edge` 荣誉，不回收飞行格、不触发 BOOST、不改变主线。旧 `target.ring`、
  `target.center` 只作为历史迁移 id 保留。
- 失败结算先展示带撞柱 / 偏离米数 / 起飞高度与建议的失败回顾，再进入高光墙；成功结算先展示
  Final 认证，再展示高光墙。高光墙的“继续下一轮”是默认焦点，5 秒倒计时自动继续并保留七飞进度；
...
发布前用两只实体手柄从玩法目录进入“双打”：分别按左 / 右入座，确认两边完整摇杆向量、右侧飞行
白雾航道在分屏中持续存在；再淘汰一席，确认两侧窗口都跟随幸存者且淘汰席可以安全触发支援 / 追踪鸭。
最后完成一局并检查失败回顾、高光墙和自动续跑在桌面与横屏手机上的比例。
