...
状态：主分支当前工作包已圆满交付“后方与侧后方高空上帝视角跟进运镜、黄金跟焦距离实拍核验、高光横幅天际居顶优化与分数整数格式化（Rear & Rear-Diagonal Elevated God's-Eye Follow & Verified Camera Framing）”。

## 当前工作包

- **后方与侧后方高空上帝视角跟进运镜（Rear & Rear-Diagonal Elevated God's-Eye Follow）**：
  1. **3 段式后方/侧后方高空追焦运镜矩阵**：
     - **Angle 1（0.0s – 2.8s · 侧后方45°高空俯瞰跟进）**：距离 $7.5\text{m}$，高度 $4.2\text{m}$，视场角 $58^\circ$，侧后方 $25^\circ$ 俯角跟拍，将玩家快艇、赛手身姿、尾翼扰流板与身旁并驾齐逐的对手船只完美纳入黄金构图；
     - **Angle 2（2.8s – 5.4s · 侧后方弧形慢镜头特写俯瞰）**：距离 $6.6\text{m}$，高度 $3.6\text{m}$，视场角 $54^\circ$，$0.35\times$ 子弹时间围绕高光顶点在侧后方弧形掠过，极具电竞大片冲击力；
     - **Angle 3（5.4s – 7.5s · 正后方高空极速追焦冲刺）**：距离 $7.8\text{m}$，高度 $3.9\text{m}$，视场角 $60^\circ$，正后方高位下压俯冲追送，快艇绝尘冲向远方海平线；
  2. **高光横幅天际居顶优化（Top Banner Layout）**：高光特技横幅调整至上方天际（`top: clamp(48px, 9.5vh, 72px)`），避免遮挡屏幕中央的赛艇 3D 动作与激烈对抗；
  3. **荣耀点小数取整（Integer Stunt Score）**：彻底修复浮点小数显示（`Math.round(stuntScore)`），展示纯正街机荣耀点；
  4. **移动端窄屏排版自适应**：在 $844\times 390$ 横屏下精简 REC 标签与标题间距，杜绝换行重叠。
