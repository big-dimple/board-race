...
唯一下一步是本机实跑一局单人，按三段验收：
1. 冲线：过 Final 门看定格画面，车手应保持驾驶握把姿势，不再出现挥手 / 举臂循环；镜头与 Final
   火花照旧。
2. 荣誉目标：从 GO 起跑到第一个 checkpoint 门，故意擦撞浮漂确认黄鸭气球照旧旋转升空并爆成羽毛、
   浮标落水归位，且全程不再弹出 `鸭鸭爆点 +120` 卡片；再绕到中段金币扇区确认拾取卡只剩
   `金币猎手` / `COIN COMBO ×N` 一种形态。
3. 名次：跑满七飞后再飞第八门，名次条不应跳到第六。
三项都过后再进行原有的双打实机复核（两只实体手柄入座、摇杆回中、斜向入座、起飞边沿、暂停和震动反馈）。
