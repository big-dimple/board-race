...
- 视觉语法固定为 `半透明青色虚拟航道 -> 绿色软回收漏斗/间断箭头 -> 随浪起伏的半透明绿色薄雾主线`。
  青色航道用低透明面、扫描节奏、细墨边和白色流光融入赛璐璐海面，不得恢复紫色
  能量管或连续实体墙。门后提前露出主线并保持 `16m` 交叠；世界中仍然最多一个
  active branch。回收提示只是可视导航，不是碰撞墙，也不改变判定范围。
- 水面主线横向细分后逐顶点采样实时浪高；禁止恢复粗双轨、横杆或 fragment shader
  重复 V 字。普通路段每 `24m` 一个开放箭头，急弯以单组三联暖色尖角替代；薄雾和
  箭头都不进入 bloom，也不能复制空中通道的平板几何。
- 判定所有权与视觉所有权允许短暂分离：passed branch 在下降、触水、authored handoff
  完成前仍负责 validation；第一次真实触水后，屏幕上唯一可见 branch 可以提前切到
  下一飞，让玩家隔着浪峰预读。第七飞触水后不得错误预览第一飞。

### 第四、第五飞的可读性

- 不抬高全局飞行高度、门、船体包络或镜头。第四飞只在当前门上方增加不受海面
  遮挡的细杆 + 空心青色菱形 locator；它属于当前唯一 branch，不生成第二目标。
...
- 第五飞空中急弯固定为右转。青色分支外侧使用三个随浪浮动、由小浮漂和细杆支撑的
  ...
  hit zone、显示额外文字卡或增加第二次震动。
- 青色只承担“可飞行空间”语义，材质语言是半透明虚拟面、青白风痕、泡沫节奏和
  细墨结构；禁止恢复紫色能量管、连续实体墙或 bloom 堆亮。黄/白 action marker 靠形状区分，颜色
...
- 手机默认触控转向并在 `GO` 后直接就绪；只有玩家主动切到重力转向才请求传感器权限并校准。
  倾斜 / 触控切换只允许改变左拇指区，右侧漂移与飞行位置不能移动。
...
- 整圈绿色水面主线采用开放式贴浪格栅：连续底色 alpha 必须为 `0`，只允许双细轨、
  稀疏横格和顺行尖角。七条空中分支保留既有半透明烟雾飘带与 energy bloom，不得因
  水面主线改版而降可见度；暖色急弯提示和实体三角牌都只是空中飘带上的次级确认。
