...
- 天空 dome 保持三段渐变；8 个近云使用 `256x160` 近景贴图，8 个远云使用 `512x220`
  宽幅低对比空气贴图与 `.78` 不透明度，远云不得复用近云图标或形成一排同尺寸卡片。
  可见太阳使用独立的 `uSunVisualDir`，只改变天空中的偶发可见位置，不改变共享
  `SUN_DIR` 对艇体、车手和水面的受光；太阳必须是小圆盘、克制环光和导数无关的连续短射线，
  不得恢复等距矩形旋转符号。
...
- 第 `1/2/4/5/6` 飞和轻碰撞保持静默，不再播“艇况正常”等填充句。重碰只播有明确
  ...
  Gemini 技巧固定为 `Gemini // 线路读懂了` / `空刹压住速度，转向咬住弯心`，未掌握
