...
- 左右席的私有航道即使另一席已经激活 Final 也必须保留到本席完成 recovery；只有本席回到水面且没有活跃飞行分支时，
  才可切换为 Final 接近提示。手柄断连会冻结双打 fixed-step，重连后必须由已入座设备产生新的确认边沿才恢复。
...
- 另外八个大型荣誉目标（两只鸭子、六枚金币）由 `HonorTargetSystem` 负责
  ...
  信号桅杆和实心徽记组成；金币有厚度、双面轮缘和压印，并且只放在离开起终点缓冲区的中段水面扇区，
  绝不伪装成升空菱形或隐藏资源站。它们不直接改写 Boat 的世界变换、
  飞行库存或路线真相。命中半径内再区分 `center` / `edge`，两种精度都只写入对应稳定荣誉 id；
  每个目标对每名赛车手只记一次，事件进入预分配池，避免在 fixed-step 中无界分配。历史 ledger
  仍保留 `target.ring` / `target.center` / `target.bell` / `target.star` / `target.crown` / `target.comet`
