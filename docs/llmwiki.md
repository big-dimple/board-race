...
- 视觉语法固定为 `绿色水面主线 -> 半透明青色虚拟航道 -> 青色回收尾段 -> 绿色水面主线`。
  青色航道的视觉几何必须从 surface mask 的交权点开始，即使真实计分 entry 更晚；两者之间
  不得出现绿色已隐藏而青色尚未开始的空窗。急弯暖色只允许落在 authored 尖角上，整片
...
- 七个起飞入口统一使用同一套世界内升空向量，不用字幕或 HUD 冒充路线：水面主线在
  `launchFromU` 处停止，两只低矮浮漂投影器随浪运动，三枚空心菱形沿约 22m 弧线从
  ...
  流动尖角和沿线浮标从 `launchFromU` 到 flight exit 必须连续隐藏，不能在菱形退场后
