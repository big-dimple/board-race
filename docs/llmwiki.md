...
- 另外八个大型荣誉目标（鸭子、发光环、海铃、星标、王冠、彗星）由 `HonorTargetSystem` 负责
 视觉和 fixed-step 接触采样；它们不直接改写 Boat 的世界变换或路线真相。命中半径内再区分
  `center` / `edge`：只有环心的精准穿越才会在主线程回收一格飞行能量，满格时转为一次 0.85 秒
  水面 BOOST；擦边只记对应荣誉，不伪造资源收益。每个目标对每名赛车手只记一次，事件进入预分配池，
  避免在 fixed-step 中无界分配。
