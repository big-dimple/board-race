...
- 视觉语法固定为 `青白风痕式门前轨道 -> 绿色软回收漏斗/间断箭头 -> 绿色水面主线`。
...
- 青色只承担“可飞行空间”语义，材质语言是青白风痕、泡沫节奏和细墨结构；禁止
  恢复紫色能量管、连续实体墙或 bloom 堆亮。黄/白 action marker 靠形状区分，颜色
...
- 资料片 dossier 打开时拥有全屏交互：移动端开始、模式、转向、漂移和飞行控件全部隐藏，
  返回结算后恢复；竖屏旋转阻断继续保留。
