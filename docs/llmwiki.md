...
- 艇边飞行库存和计时轨不能盖住车手。桌面根据车手投影选择相反一侧；横屏手机使用固定
  的右上安全通道，并避开右下控制。
- 空中续航反馈使用右上紧凑提示，不显示全屏闪白或速度线，不占用飞行路线中心。
...
- `ocean.ts` 仍是一张 opaque/depth-writing 海面 draw。颜色、反射和白帽由共享高度、坡度、
  垂直速度与曲率驱动；近场细纹和高质量 glint 只改材质法线。禁止用绿色、加亮、连续白轨
  或脱离物理波峰的 shader 噪声伪装“更汹涌”。
- 尾流应读成断续中央含气水带，肩浪只是次级信息；落水水花必须来自真实接触事件并在退场后归零。
