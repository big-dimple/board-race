...
- 水面浮标(checkpoint 门浮标、转弯 chevron 浮标)是实体:水面船体接触把它撞飞
  (抛物、落水、延迟归位),撞击船付一次小幅减速,不判负、不触发碰撞电台/镜头冲击;
  飞行相位与足够高度的浪跳不触发。checkpoint 浮标的实体生命周期不由飞行路线
  指引显隐裁剪；只有明确归属未激活路线的 chevron 才能随路线组隐藏。
