...
- `TeamExpedition` 不运行 `Race`，不进入排名、勋章、独立竞技纪录或 Final 结算。首版是驾驶校准加
...
- 飞行通过并完整落水后，双方必须进入各自实体泊位、速度不高于 `2.5 m/s`，共同保持能力键
...
玩法目录 -> 队伍协作 -> 进度选择 -> 按键入座 -> 双席选角
       -> first-run calibration -> resonance -> locks -> sky relay
...
`verify:team` 用真实键区完成首局驾驶校准、四次共振接力、两次水闸互换、门控飞行和低速双泊位；
同时检查玩法协议命名、左右入座、角色互斥、手动油门不串席、漏接重试、未供能拒绝起飞、
