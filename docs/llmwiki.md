...
- 另外八个大型荣誉目标（鸭子、海铃、星标、王冠、彗星，按布局复现）由 `HonorTargetSystem` 负责
  视觉和 fixed-step 接触采样；每个目标都是水面实体浮标，根节点贴着实时浪高，由浮筒、泡沫圈、
  信号桅杆和实心徽记组成，绝不伪装成升空菱形或隐藏资源站。它们不直接改写 Boat 的世界变换、
  飞行库存或路线真相。命中半径内再区分 `center` / `edge`，两种精度都只写入对应稳定荣誉 id；
  每个目标对每名赛车手只记一次，事件进入预分配池，避免在 fixed-step 中无界分配。历史 ledger
  仍保留 `target.ring` / `target.center` 旧 id 供迁移读取，但当前布局不会再产生这两类事件。
