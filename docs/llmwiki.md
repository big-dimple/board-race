...
- `ocean.ts` 仍是一张 opaque/depth-writing 海面 draw。颜色、蓝色天空反射和白帽由共享高度、
  ...
  hash cell 星芒碎斑构成（各层不同尺度、旋转角与 twinkle 速度，格内抖动消点阵，沿太阳方位
  各向异性拉伸成远场光路，按像素足迹逐层退场防 shimmer），在物理+涟漪法线的 Blinn 包络下
  快速聚散，高质量档增加细闪层；它只属材质表现，不改变波形或浮力。cell 距离场不连续，抗锯齿
  必须用解析像素足迹，禁止对其使用 `fwidth`（会在格界画出虚线格框）。禁止用绿色、加亮、连续
  白轨或脱离物理波峰的 shader 噪声伪装“更汹涌”。
- 太阳的可见呈现（日盘、光晕、四刺镜头十字、水平变形眩光、丁达尔光束扇）归 `sky.ts` 天空
  穹顶，只在朝向太阳方位时增强（追光读法），不改变共享光照方向，也不在海面之外另造光源。
