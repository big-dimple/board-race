...
- 视觉语法固定为 `绿色水面主线 -> 三枚升空菱形 -> 半透明青色虚拟航道 -> 青色回收尾段 -> 绿色水面主线`。
  绿色水面主线负责到 `launchCueU`，三枚菱形负责从水面姿态交向空中姿态，七条青色航道的
  mesh 则必须精确从各自真实 `entryU` 开始。禁止在入口前铺一段不参与起飞、碰撞或计分却像
  可驾驶水道的青色面 / 云桥；surface mask 仍从 launch cue 开始，不能为了填空重新前移。
  ...
  新青色 branch；触水本身仍不是换材质或换视觉所有权的时机。七条青色航道统一使用
  `cel-virtual-corridor`：深青透明主体、青白双流线、屏幕空间稳定的细墨边三层分工，
  不能再用同一种青色只靠透明度区分。基础 alpha 为主体 `.17`、扫描节奏 `.085`、中心
  `.09`、边缘 `.42`、流线 `.62`；`55-145m` 仅增强边缘和流线，主体最多额外 `.04`，
...
- 青色只承担“可飞行空间”语义，材质语言是可持续辨认的半透明虚拟面、青白风痕、
...
- 宽日照之外允许一层强度 `.5` 的短促方向波光：它必须来自连续细法线，并以
  `fwidth(glintField)` 做导数过滤，只在 `10-34m` 渐入、`230-430m` 渐出。旧的高度色块、
  量化闪点、hash 像素噪声和菱形符号仍然退役；这层波光不能占满近船动作区或远景。
