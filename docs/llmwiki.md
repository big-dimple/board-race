...
- `gate / gate_left / gate_right` 都向玩家显示“撞柱”；左右偏差只作为次级数字证据。
  ...
  或反弹横向投影 0.5s 仍留在白雾航道硬边界内（出不了赛道）的接触记 `spared`——本次飞行尝试
  照常失败、不淘汰，船保留动量反弹（不触发弹球倒摔）、翻滚落水后按 AI 同款
  `recoverFailedFlightRoute` 下一圈重试，表现层打出「身残志坚」逐字喜剧字幕
  （`hud-grit`：身残占左侧电台列、志坚占右侧飞控件上方，永远不遮中央竞速视野，
  双打按席半屏归位）；只有深撞柱心被弹出航道的接触维持原弹球倒摔 + 淘汰流程。
...
- 「身残志坚」字幕（放过撞柱的喜剧反馈）逐字蹦出：毛笔展示字（自托管 4 字形
  Ma Shan Zheng 子集 + 系统楷体栈回落）、中央电影字幕卡——半透明羽化墨带底衬
  （峰值约 0.5 不透明度）+ 宣纸米白渐变题字 + 暖金柔晕，与 HUD 漫画
  语言一致；屏幕中轴约 33% 高度带（警示横幅之下、船体投影之上），
  移动端上移约 27% 并避开触控热区，双打整卡按席归半屏中轴；
  逐字重砸 stagger 0.14s、总时长 3.4s，带「撞柱 · 大难不死」kicker
  与朱红印章；结果层/勋章/复盘/
