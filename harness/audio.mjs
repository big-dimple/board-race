...
try {
  ...
  assert.equal(Number(state.startSignalEvents), signalBeforeGo + 1, 'GO must emit exactly one start signal');
  assert.ok(Number(state.musicFilterHz) > 4200, `GO must open score filter: ${state.musicFilterHz}`);
  assert.ok(Number(state.musicDuck) <= 0.55, `GO signal should briefly clear score: ${state.musicDuck}`);
  ...
  await advanceWithWallClock(mobilePage, 9.4);
