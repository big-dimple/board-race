...
const FLIGHT_FOV = [78, 79, 80, 81, 82, 83, 84] as const;
...
export class CameraRig {
  ...
  update(dt: number, boat: IBoat, t: number): void {
    ...
    if (this.activeMode === 'chase') {
      ...
      this.heaveAnchor += (frameY - this.heaveAnchor) * (1 - Math.exp(-dt / HEAVE_TAU));
      ...
      const altitudeBack = Math.min(1.2, Math.max(0, st.flightClearance - 4.5) * 0.08);
      target.set(
      ...
      const ax = frameX + fx * this.lookAhead;
      const az = frameZ + fz * this.lookAhead;
      look.set(
        ax,
