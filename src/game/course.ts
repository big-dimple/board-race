...
export class Course implements ICourse {
  ...
  private readonly balloonPops: { x: number; y: number; z: number }[] = [];
  ...
  consumeBalloonPops(out: { x: number; y: number; z: number }[]): { x: number; y: number; z: number }[] {
    out.length = 0;
    for (const p of this.balloonPops) {
      out.push(p);
    }
    this.balloonPops.length = 0;
    return out;
  }
    ...
    for (const f of this.floaters) {
      if (f.balloonFlight && !f.balloonFlight.popped && f.balloonObj) {
        ...
        if (b.timer >= BALLOON_POP_TIME) {
          ...
          this.balloonPops.push({ x: b.x, y: b.y, z: b.z });
        } else {
  ...
  private buildGates(): void {
    ...
    const makeDuckBalloon = (): THREE.Group => {
      ...
      tether.position.y = -0.08;
      tether.userData.noOutline = true;

      g.add(body, head, beak, wingL, wingR, tether);
      addOutline(g, { width: 0.75 });
      markInk(g);
      tether.layers.disable(LAYER_INK);
      return g;
    };
