/**
 * Procedural rider skin with a rigid closed helmet.
 *
 * The animation rig remains code-driven in rider.ts. This module turns that
 * rig into one body SkinnedMesh; the head is NOT part of it — identity lives
 * in a rigid helmet (helmet.ts) hard-bound to the `head` bone: no facial
 * features, no portrait face patch, no hair skeleton, no spring bones. The
 * portrait-era face/hair system was removed for the H5 budget: helmet
 * vertices never run the skinning path, and there is zero per-frame head
 * accessory math. Identity is helmet-silhouette + paint: per-driver helmet
 * style, saturated team paint with a high-contrast rear stripe, white racing
 * suit with team-color gloves and piping.
 */
...
class SkinAssembler {
  ...
  private readonly _localPosition = new THREE.Vector3();
  private readonly _position = new THREE.Vector3();
  private readonly _normal = new THREE.Vector3();
  private readonly _matrix = new THREE.Matrix4();
  private readonly _boneMatrix = new THREE.Matrix4();
  private readonly _rootInverse = new THREE.Matrix4();
  ...
  constructor(
    ...
    this._rootInverse.copy(root.matrixWorld).invert();
  }
  ...
  append(
    ...
    this._boneMatrix.multiplyMatrices(this._rootInverse, bone.matrixWorld);
    this._matrix.multiplyMatrices(this._boneMatrix, localMatrix);
    this._normalMatrix.getNormalMatrix(this._matrix);

    for (let i = 0; i < pos.count; i++) {
      this._localPosition.fromBufferAttribute(pos, i).applyMatrix4(localMatrix);
      this._position.copy(this._localPosition).applyMatrix4(this._boneMatrix);
      this._normal.fromBufferAttribute(normal, i).applyMatrix3(this._normalMatrix).normalize();
      this.positions.push(this._position.x, this._position.y, this._position.z);
      this.normals.push(this._normal.x, this._normal.y, this._normal.z);

      const vertexRole = typeof role === 'function' ? role(this._localPosition) : role;
      ...
      const vertexWeights = weightFn?.(this._localPosition) ?? [[bone, 1]];
      let total = 0;
...
export function disposeRiderSkin(skin: RiderSkin): void {
  ...
  if (skin.helmet) disposeHelmet(skin.helmet);
}
...
export function buildSkinnedRider(
  ...
  // Neck base only — the head itself is the rigid helmet's job (helmet.ts),
  // hard-bound to rig.head. +Z is face-forward in head-bone space.
  ...
  const helmet = buildHelmet(rig.head, look, color, detailed);
  return { mesh, roles: result.roles, colorAttribute: result.colorAttribute, helmet };
}
...
export function updateSkinnedRiderLook(skin: RiderSkin, color: number, look: RiderLook): void {
  ...
  // Driver switch = helmet style switch: rebuild the rigid helmet. Geometry
  // and materials are module-cached and shared, so this only re-parents
  // lightweight meshes — the same disposal contract the hair system used.
  if (skin.helmet && skin.helmet.driverId !== look.driverId) {
    const head = skin.helmet.object.parent as THREE.Bone;
    const next = buildHelmet(head, look, color, skin.helmet.detailed);
    disposeHelmet(skin.helmet);
    skin.helmet = next;
    if (next.detailed) {
